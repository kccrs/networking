    ...
    return (
      <article
        ...
        <div className="ShortText" onClick={this.props.handleClick}>
          ...
          <p className="Company"><span>Company: </span> {this.props.company}</p>
        </div>
        <div className="followUpToggle">
        <button className="followUpMessage">
          Follow Up -->
        </button>
        <button className="followUpIcon" name="Follow Up" value={this.props.followUp}
          onClick={() => this.updateProperty('followUp', this.props.followUp)}>
        </button>
        </div>
      </article>
    );
