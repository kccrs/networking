...
export default class ContactList extends Component {
  ...
  updateRecord(key, property, newValue) {
    this.reference.update({followUp: newValue});
  }

  render() {
    const contacts = this.state.contacts.map((contact) => <ShortContact {...contact} reference={this.reference.child(contact.key)} updateFunction={this.updateRecord} />);
